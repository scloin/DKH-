...
Layer::Layer(int M, int N, int O, char* S) {
    this->M = M;
    this->N = N;
    this->O = O;
	this->S = S;
    ...
    // Assuming the JSON has keys like "layer1", "layer2" for different layers
    json layer_data = model_data[S];

    // Load bias and weight from JSON
    std::vector<float> h_bias = layer_data["bias"].get<std::vector<float>>();
    std::vector<std::vector<float>> h_weight = layer_data["weight"].get<std::vector<std::vector<float>>>();
    ...
    //Flatten the 2D weight array into a 1D array for copying
	float *h_pweight=(float *)malloc(sizeof(float) * M  * N);
	float *h_pbias=(float *)malloc(sizeof(float) * N);
	int i =0;
    for (const auto& row : h_weight) {
	...
	i =0;
	for (const auto& val : h_bias){
		h_pbias[i]=val;
		i++;
	}
    cudaMemcpy(weight, h_pweight, sizeof(float) * M * N, cudaMemcpyHostToDevice);
    cudaMemcpy(weight, h_pbias, sizeof(float) * M * N, cudaMemcpyDeviceToHost);
}
...
void Layer::save_to_json() {
    // Read existing JSON file (if exists)
    std::ifstream infile("model1.json");
    json model_data;
    if (infile.is_open()) {
        infile >> model_data;
        infile.close();
    ...
    // Retrieve bias data from GPU
    std::vector<float> h_bias(N);
    cudaMemcpy(h_bias.data(), bias, sizeof(float) * N, cudaMemcpyDeviceToHost);

    // Retrieve weight data from GPU
    std::vector<float> h_weight(M * N);
    cudaMemcpy(h_weight.data(), weight, sizeof(float) * M * N, cudaMemcpyDeviceToHost);

    // Convert weight to 2D vector
    std::vector<std::vector<float>> h_weight_2d(N, std::vector<float>(M));
    for (int i = 0; i < N; ++i) {
        for (int j = 0; j < M; ++j) {
            h_weight_2d[i][j] = h_weight[i * M + j];
        }
    }

    // Add or update layer data in the JSON object
    model_data[S] = {
        {"bias", h_bias},
        {"weight", h_weight_2d}
    };
    ...
    std::ofstream outfile("model1.json");
    outfile << model_data.dump(4);  // Pretty-print with 4-space indent
    outfile.close();
}
