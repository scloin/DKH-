...
static Layer l_input = Layer(0, 0, 28*28,"l_input");
static Layer l_c1 = Layer(5*5, 6, 24*24*6,"l_c1");
static Layer l_s1 = Layer(4*4, 1, 6*6*6, "l_s1");
static Layer l_f = Layer(6*6*6, 10, 10, "l_f");
...
static void test()
{
	...
	fprintf(stdout, "Accuracy: %.2lf%%\n",
		100.0 - double(error) / double(test_cnt) * 100.0);
}
